...
function! s:errorformat() abort
  ...
  let l:goroot = go#util#goroot()

  let l:show_name = go#config#TestShowName()
  if s:efm != '' && s:go_test_show_name is show_name
    return s:efm
  endif
  let s:go_test_show_name = l:show_name
  ...
  if l:show_name
    let l:format .= ',%G' . l:indent . '%#--- FAIL: %m (%.%#)'
  else
    let l:format .= ',%-G' . l:indent . '%#--- FAIL: %.%#'
  endif
  ...
  " set the l:format for panics.

  " handle panics from test timeouts
  let l:format .= ',%+Gpanic: test timed out after %.%\+'
  ...
  let l:format .= ',%+Afatal error: %.%# [recovered]'
  let l:format .= ',%+Apanic: %.%# [recovered]'
  let l:format .= ',%+Afatal error: %.%#'
  let l:format .= ',%+Apanic: %.%#'
  ...
  " inside GOROOT.
  let stdlibaddress = '%\t' . l:goroot . '%\f%\+:%\d%\+ +0x%[0-9A-Fa-f]%\+'

  " Match and ignore the running goroutine line.
  let l:format .= ',%-Cgoroutine %\d%\+ [running]:'
  ...
  let l:format .= ',%-Cexit status %[0-9]%\+'
  "let l:format .= ',exit status %[0-9]%\+'
  ...
  let l:format .= ',%A%f:%l:%c: %m'
  let l:format .= ',%A%f:%l: %m'
